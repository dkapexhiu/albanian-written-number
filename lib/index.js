"use strict";
exports = module.exports = albWordToNum;
...
function albWordToNum(n, options) {
  options = options || {};
  options = util.defaults(options, albWordToNum.defaults);
  ...
  if (!language) {
    if (languages.indexOf(albWordToNum.defaults.lang) < 0) {
      albWordToNum.defaults.lang = "al";
    }

    language = i18n[albWordToNum.defaults.lang];
  }
  ...
  if (m) {
    if (
      ...
      ret.push(albWordToNum(m, options));
    } else {
      ret.push(language.unitSeparator + albWordToNum(m, options));
    }
  }
...
function handleSmallerThan100(n, language, unit, baseCardinals, alternativeBaseCardinals, options) {
  ...
  if (unit) {
    return (
      alternativeBaseCardinals[dec] || baseCardinals[dec] + language.baseSeparator + albWordToNum(unit, options)
    );
  }
